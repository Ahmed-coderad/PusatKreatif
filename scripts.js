...
const AudioFX = (() => {
    ...
    return {
        ...
        // Sound effects are a permanent part of the experience: every
        // animated interaction must produce its matching sound, so the
        // engine can never be switched off from anywhere in the app.
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const sounded = document.querySelectorAll('.button, .menu button, .video-sound-button, .translate-button, .gift-box-btn, .modal-buttons button');
    sounded.forEach(el => {
...
/* --------------------------------------------------------------------------
   Video sound button — lives directly on the video and toggles its audio
   on/off, in place of the separate row it used to sit in.
   -------------------------------------------------------------------------- */
function toggleVideoSound() {
    ...
    const label = video.muted
        ? (currentUiLang === 'en' ? 'Turn on video sound' : 'Aktifkan suara video')
        : (currentUiLang === 'en' ? 'Turn off video sound' : 'Matikan suara video');
    btn.setAttribute('aria-label', label);
    btn.setAttribute('title', label);
}

/* --------------------------------------------------------------------------
   Bilingual copy — extended to cover the gift vault + video sound button.
   -------------------------------------------------------------------------- */
...
function toggleTranslation() {
    ...
    currentUiLang = isEnglish ? 'en' : 'id';

    const videoMuted = !video || video.muted;
    const videoLabel = isEnglish
        ? (videoMuted ? 'Turn on video sound' : 'Turn off video sound')
        : (videoMuted ? 'Aktifkan suara video' : 'Matikan suara video');
    if (unmuteButton) {
        unmuteButton.setAttribute('aria-label', videoLabel);
        unmuteButton.setAttribute('title', videoLabel);
    }
