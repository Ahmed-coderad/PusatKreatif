...
/* --------------------------------------------------------------------------
   Sound indicator — sound effects are always on and cannot be muted.
   Every animated interaction (hover, click, menu open, gift reveal, etc.)
   automatically plays its matching sound. The button is kept only as a
   visual confirmation that sound is active, not as a mute switch.
   -------------------------------------------------------------------------- */
function setupSoundToggle() {
    ...
    btn.disabled = false; // keep it focusable/hoverable for the cue sound
    // A tap gives an audible confirmation instead of muting anything.
    btn.addEventListener('click', () => AudioFX.chime());
}
...
function toggleTranslation() {
    ...
    if (isEnglish) {
        if (soundToggle) {
            soundToggle.setAttribute('aria-label', 'Sound effects are always on');
            soundToggle.setAttribute('title', 'Sound effects are always on');
        }
        ...
        if (soundToggle) {
            soundToggle.setAttribute('aria-label', 'Efek suara selalu aktif');
            soundToggle.setAttribute('title', 'Efek suara selalu aktif');
        }
